...
def update_user_profile(user_id):
  if current_user.department == 3:
    ...
    SELECT
        ...
        name ,
        altMobile     
    FROM User_data user WHERE id = %s''',[user_id])
    user_info = list(db.fetchall()[0])
    if request.method == 'POST':
        if (
            request.form['email']==user_info[2]
            and request.form['mobile']==user_info[3]
    ...
    zones = db.fetchall()
    conn.close()
    return render_template('update_profile.html', user=user_info,zones=zones)
  else:
      return redirect(url_for('home_blueprint.index'))

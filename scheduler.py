...
def scheduledTask():
    ...
    info = db.execute('''SELECT
         ...
         pending_intervals,invoices.id
          ...
          WHERE status = "Active" and invoices.next_invoice_date between ? and ?
          AND processed != 1
          ORDER BY next_invoice_date ASC
          ''',[last_success_date,today]).fetchall()
    # print(len(info))
    for i in range(len(info)):
        ...
        db.execute('''UPDATE balance_info SET due_amount = due_amount + ?, one_interval_amount = ?, pending_intervals = ? WHERE user_id = ?
